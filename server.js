...
app.post(
    "/getlastversion",
    jsonParser,
    function (request, response_getlastversion = response) {
...
app.post(
    "/generate",
    jsonParser,
    function (request, response_generate = response) {
...
app.post("/savechat", jsonParser, function (request, response) {
    ...
    var dir_name = String(request.body.avatar_url).replace(
        `.${characterFormat}`,
        ""
    );
...
app.post(
    "/getstatus",
    jsonParser,
    function (request, response_getstatus = response) {
...
app.post("/createcharacter", urlencodedParser, function (request, response) {
    ...
    if (
        !fs.existsSync(
            charactersPath + request.body.ch_name + `.${characterFormat}`
        )
    ) {
        if (!fs.existsSync(chatsPath + request.body.ch_name))
            fs.mkdirSync(chatsPath + request.body.ch_name);
    ...
    } //console.log(request.body); //response.send(request.body.ch_name);
...
app.post("/editcharacter", urlencodedParser, function (request, response) {
    ...
    if (!filedata) {
        charaWrite(
            img_path + request.body.avatar_url,
            char,
            target_img,
            characterFormat,
            response,
...
app.post("/deletecharacter", urlencodedParser, function (request, response) {
    ...
    rimraf(charactersPath + request.body.avatar_url, (err) => {
        if (err) {
            ...
            rimraf(
                chatsPath + dir_name.replace(`.${characterFormat}`, ""),
                (err) => {
                    if (err) {
                        response.send(err);
                        return console.log(err);
...
async function charaWrite(
    ...
    try {
        ...
        switch (format) {
                ...
                return new Promise(async (resolve, reject) => {
                    ...
                    try {
                        ...
                        const processedImage = await sharp(imageBuffer)
                            ...
                            .toBuffer();
                        fs.writeFileSync(
                            charactersPath + target_img + ".webp",
                            processedImage
                        );
                        resolve({});
                        if (response !== undefined) response.send(mes);
            ...
            case "png":
                var image = await sharp(source_img)
                    .resize(400, 600)
                    .toFormat("png")
                    .toBuffer(); // old 170 234
                // Convert the image to PNG format
                //const pngImage = image.toFormat('png');
                ...
                // Add new chunks before the IEND chunk
                var base64EncodedData = Buffer.from(data, "utf8").toString(
                    "base64"
                );
                chunks.splice(
                    -1,
                    0,
                    PNGtext.encode("chara", base64EncodedData)
                );
                //chunks.splice(-1, 0, text.encode('lorem', 'ipsum'));
        ...
        for (const item of imgFiles) {
            ...
            try {
                ...
                if (error instanceof SyntaxError) {
                    ...
                    console.error(
                        "An unexpected error loading character index " +
                            i +
                            " occurred.",
                        error
                    );
...
app.post("/savestyle", jsonParser, function (request, response) {
    ...
    if (this_style != "classic.css") {
        file_data =
            '@import "../designs/classic.css";@import "../designs/' +
            this_style +
            '";';
    }
...
app.post(
    "/generate_novelai",
    jsonParser,
    function (request, response_generate_novel = response) {
...
app.post(
    ...
    function (request, response_getstatus_openai = response) {
        ...
        client
            .get(
                api_openai + "/engines/text-davinci-003",
                args,
                function (data, response) {
                    if (response.statusCode == 200) {
...
app.post(
    ...
    function (request, response_generate_openai) {
        ...
        if (
            ...
            request.body.model === "gpt-3.5-turbo-0301"
        ) {
        ...
        client
            .post(api_openai + request_path, args, function (data, response) {
                try {
                    if (
                        request.body.model === "gpt-3.5-turbo" ||
                        request.body.model === "gpt-3.5-turbo-0301"
...
app.post(
    ...
    async function (request, response) {
        ...
        let img_name = "";
        let filedata = request.file; //console.log(filedata.filename);
            ...
            fs.readFile(
                ...
                (err, data) => {
                    ...
                    if (jsonData.chat && Array.isArray(jsonData.chat)) {
                        ...
                        i++;
                        jsonData.histories.histories[0].msgs.forEach(function (
                            item
                        ) {
                            new_chat[i] = {};
...
//********CHARACLOUD API***********//
app.post(
    "/characloud_getallcharacters",
    jsonParser,
    function (request, response_characloud_getallcharacters = response) {
        try {
            client
                .get(
                    charaCloudServer + "/characters",
                    function (data, response) {
                        if (response.statusCode === 200) {
                            response_characloud_getallcharacters.send(data);
                        } else {
                            response_characloud_getallcharacters.sendStatus(
                                500
                            );
                        }
                    }
...
app.post(
    "/characloud_loadcard",
    jsonParser,
    function (request, response_characloud_loadcard = response) {
            ...
            .get(url, (response) => {
                ...
                file.on("finish", async function () {
                    ...
                    try {
                        ...
                        charaWrite(
                            filename,
                            JSON.stringify(charaFormatData(char)),
                            setCardName(char.name),
                            characterFormat,
                            response_characloud_loadcard,
                            { send: "ok" }
                        );
                    } catch (error) {
                        if (error instanceof SyntaxError) {
...
app.post(
    "/characloud_search_character",
    jsonParser,
    function (request, response_characloud_search) {
        let search_string = request.body.q;
        try {
            client
                .get(
                    charaCloudServer + "/characters?q=" + search_string,
                    function (data, response) {
                        if (response.statusCode === 200) {
...
app.post(
    "/characloud_serverstatus",
    jsonParser,
    function (request, response_characloud_server_status) {
        try {
            client
                .get(
                    charaCloudServer + "/server/status",
                    function (data, response) {
                        if (response.statusCode === 200) {
